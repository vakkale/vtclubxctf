...
const SideBar: FC<SideBarProps> = ({ location, navigate, size, sideBarText, subPages }) => {
	...
	useEffect(() => {
		...
		const matchingPage = subPages.find((page) => page.url.split("/").pop() === lastPath);
	...
	return (
		<div className="sidebar-container" id="sidebar" style={{
			flex: size === "big" ? "calc(100% * (1 / 3))" : "25%",
		}}>
			{subPages && subPages.length > 0 &&
				<>
					<FeaturedItem title={currentPage?.title || ""} date={currentPage?.date || ""} category={currentPage?.category || ""}></FeaturedItem>
					<ul>
